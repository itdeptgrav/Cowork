...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    answer:
      "Offline is never chosen for you arbitrarily — it is what happens when the thing that made you Online stops. Stopping the share from the browser's own bar, losing the room connection for good, closing the tab, or and closing the tab all return you to Offline. Stopping a screen share does NOT: since Online became a choice rather than a consequence of sharing, ending a share leaves you online and not sharing. A brief network wobble does not: if your connection drops for a moment — wifi blinking, switching network, a laptop sleeping for a second — the status menu says “Reconnecting to the room…” and you stay as you were, because the screen share has not actually stopped and nobody has observed you leave. It reconnects on its own and the message clears. Only when the connection is genuinely lost, rather than briefly interrupted, do you go Offline. That is a change: a momentary drop used to show you as Offline and then Online again a second later, which everyone watching could see. The status menu always states the reason underneath, so Offline is never left to be inferred.  Refreshing the page does not do that. A refresh gives the tab a new identity, and the tab that is actually there takes over keeping your status alive within about a minute and a half — so a reload leaves you online, as it should. If your computer crashes, sleeps or is closed without a chance to sign out, your status goes offline on its own shortly afterwards — a live session keeps proving it is there, and one that stops proving it stops counting as online. That is why nobody is left showing as online overnight. Having several tabs open is fine, and every one of them shows the same status. Choosing Offline yourself asks you twice, on purpose. First a short confirmation — “Go offline? Your team sees you leave, and any running timer stops” — with “Yes, go offline” and “Stay online”. That step exists because the row sits in a menu one press away from your status pill, and a stray click used to end somebody's day outright. Only after you confirm does it open the end-of-day report over the tasks your timer ran on that day, and you go offline whether you fill it in or skip it.",
    related: [
      "status-online",
      "status-entire-screen",
      "status-task-actions",
