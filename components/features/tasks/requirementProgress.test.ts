...
/* Resolved per-machine, and with CRLF normalised, by `backendSource`. */
const ROUTE = "routes/task_routes/requirementProgress.routes.js";
...
test("it is stored in its own collection, not on the task", () => {
  ...
  const route = backendSource(ROUTE);
